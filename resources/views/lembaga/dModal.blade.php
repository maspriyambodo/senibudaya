<div id="dModal" class="modal fade show" tabindex="-1" role="dialog" aria-labelledby="dModalTitle" aria-modal="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form id="del_form" action="{{ url('lembaga/update') }}" novalidate="novalidate" method="POST" autocomplete="off">
                @csrf
                @method('POST')
                ...
                <div class="modal-footer">
                    ...
                    <button id="dbtn_submit" type="button" class="btn btn-danger" onclick="lembagaDelete();">Hapus</button>
                </div>
            </form>
        </div>
    </div>
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                if (data.success) {
                    ...
                    $('#dModalTitle').html('Hapus ' + data.dt_lembaga.hasil[0].lembaga_seni.nama);
                    $('#dModal').modal('show');
                    Swal.close();
                } else {
                    Swal.fire({
