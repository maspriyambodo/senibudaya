...
{
    private $target = 'cms.contact.complaint';
            ...
            if (request()->has('keyword')) {
                if(strlen($filter[0]) < 1) {
                    $query->where(function ($data) {
                        $data->where('nama_pengaduan', 'like', "%" . request('keyword') . "%")
                        ->orWhere('email_pengaduan', 'like', "%" . request('keyword') . "%");
                    });
                }

                if(in_array("0", $filter)) {
                    $query->where('nama_pengaduan', 'like', "%" . request('keyword') . "%");
                }
                if(in_array("1", $filter)) {
                    $query->where('email_pengaduan', 'like', "%" . request('keyword') . "%");
                }
            }
        })
        ->rawColumns(['button'])
    ...
    public function index()
    {
        ...
        'order' => array("[3, 'asc']"),
        'data' => array('button', 'nama_pengaduan', 'email_pengaduan', 'created_at'),
        'nosort' => array(0,1),
        );
    ...
    {
        $pengaduan = Pengaduan::find($request->dt);
        $pengaduan->delete();
